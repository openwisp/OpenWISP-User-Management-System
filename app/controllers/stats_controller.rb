class StatsController < ApplicationController
  ...
  def account_stat_data(id)
    case id
      when 'account_logins' then current_account.session_times_from(14.days.ago)
      when 'account_traffic' then current_account.traffic_sessions_from(14.days.ago)
  ...
  def operator_stat_data(id)
    case id
      when 'user_logins' then User.find(params[:user_id]).session_times_from(14.days.ago)
      when 'user_traffic' then User.find(params[:user_id]).traffic_sessions_from(14.days.ago)
      when 'registered_users' then User.registered_each_day_from(14.days.ago)
      when 'traffic' then RadiusAccounting.traffic_each_day_from(14.days.ago)
      when 'logins' then RadiusAccounting.logins_each_day_from(14.days.ago)
