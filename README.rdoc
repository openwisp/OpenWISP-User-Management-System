...
To do that proceed by copying the file *config/registration_fields.example.yml* and renaming it to *config/registration_fields.yml*, then set to *false* the fields you want to disable and hide.
...
If the file *config/registration_fields.yml* does not exist, OWUMS will read the default values from *config/registration_fields.example.yml* (so don't delete it).
...
1. *New account notification*: when they register and verify their new account
2. *Password reset*: when they reset their password

This means that the rails SMTP server must be configured correctly otherwise the owums will not work properly.

===== New account notification

This feature might be disabled by setting *send_email_notification_to_users* to "false" in *config/registration_fields.yml*.
