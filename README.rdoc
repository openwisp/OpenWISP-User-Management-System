...
To do that proceed by copying the file *config/registration_fields.default.yml* and rename it to *config/registration_fields.yml*, then set to *false* the fields you want to disable and hide.
...
If the file *config/registration_fields.yml* does not exist, OWUMS will read the default values from *config/registration_fields.default.yml* (so do not delete it).
